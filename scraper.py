...
def generate_results(test=False, count=5, sort='highscore', reverse=False ):
    kwargs = {
    ...
    if sort == 'highscore':
        ...
        data = sorted(data, key=itemgetter(usort),
                           reverse=(not reverse))
...
def get_geojson(result):
    ...
    geojson['properties'].setdefault('marker-color', marker_color)
    return geojson
...
if __name__ == '__main__':
    ...
    for result in generate_results(test, args.count, args.sort, args.reverse):
        geo_result = get_geojson(result)
        pprint.pprint(geo_result)
        total_result['features'].append(geo_result)
    with open('my_map3.json', 'w') as fh:
